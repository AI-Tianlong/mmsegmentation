    ...
    for ori_img_path in tqdm(ori_img_big_list, colour='Green'):
        ...
        for img_patch_path in img_patch_list:
            if img_basename_no_suffix in img_patch_path:
                ...
                if os.path.basename(img_patch_path).split(
                        '.')[0] == img_basename_no_suffix:
                    new_big_img = img_patch
                # 处理左上的
                elif (not 'xia' in img_patch_path) and (
                        not 'you' in img_patch_path) and (not 'youxia'
                                                          in img_patch_path):
                    img_h = int(
                        os.path.basename(img_patch_path).split('_')
                        [h_index])  # [1]
                    img_w = int(
                        os.path.basename(img_patch_path).split('_')
                        [w_index])  # [2]
                    new_big_img[img_h * crop_size:img_h * crop_size +
                                crop_size,
                                img_w * crop_size:img_w * crop_size +
                                crop_size] = img_patch

                # 处理右的
                elif (not 'youxia' in img_patch_path) and ('xia'
                                                           in img_patch_path):
                    img_w = int(
                        os.path.basename(img_patch_path).split('_')
                        [w_index])  # [2]
                    new_big_img[h - crop_size:h,
                                img_w * crop_size:img_w * crop_size +
                                crop_size] = img_patch

                # 处理下的
                elif (not 'youxia' in img_patch_path) and ('you'
                                                           in img_patch_path):
                    img_h = int(
                        os.path.basename(img_patch_path).split('_')
                        [w_index])  # [2]
                    new_big_img[img_h * crop_size:img_h * crop_size +
                                crop_size, w - crop_size:w] = img_patch

                elif 'youxia' in img_patch_path:
                    new_big_img[h - crop_size:h, w - crop_size:w] = img_patch

        Image.fromarray(new_big_img).save(
            os.path.join(img_big_path, img_basename))
