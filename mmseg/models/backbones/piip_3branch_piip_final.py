    ...
    def forward(self, x):
        ...
        for idx in range(len(self.interactions)):
            ...
            if self.interact_attn_type == "deform":    
                ...
                deform_inputs["2to1"] = deform_inputs_1_vit(x1, x2, downsample_ratio1, downsample_ratio2)  # 1和2的deform输入  [[1,576 ,1,2]], [32,32], 0]  [reference_points, spatial_shapes, level_start_index]
                deform_inputs["1to2"] = deform_inputs_2_vit(x2, x1, downsample_ratio1, downsample_ratio2)  # 2和1的deform输入  [[1,1024,1,2]], [24,24], 0]  [reference_points, spatial_shapes, level_start_index]
                deform_inputs["3to2"] = deform_inputs_1_vit(x2, x3, downsample_ratio2, downsample_ratio3)  # 2和3的deform输入  [[1,1024,1,2]], [40,40], 0]  [reference_points, spatial_shapes, level_start_index]
                deform_inputs["2to3"] = deform_inputs_2_vit(x3, x2, downsample_ratio2, downsample_ratio3)  # 3和2的deform输入  [[1,1600,1,2]], [32,32], 0]  [reference_points, spatial_shapes, level_start_index]
            else:
        ...
        # Branch merging
        x1 = x1.transpose(1, 2).view(bs1, self.branch1.embed_dim, H1, W1) # [2, 576, 1024] --> [2, 1024, 24, 24]
        ...
        x2 = x2.transpose(1, 2).view(bs2, self.branch2.embed_dim, H2, W2) # [2, 1024, 768] -> [2, 768, 32, 32]
