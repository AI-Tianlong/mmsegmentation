...
class TwoBranch_backbone_mode2(BaseModule):
    ...
    def create_intermediate_merge_module(self, idx, dim1_, dim2_):
        merge_branch1 = nn.Sequential(
            nn.GroupNorm(32, dim1_),  # 32-->128
            nn.ReLU(inplace=True),
        )

        merge_branch2 = nn.Sequential(  # 将branch2的维度升到dim1
    ...
    def forward(self, x):
        ...
        if isinstance(self.branch1_backbone, ConvNeXt):
            x1 = self.branch1_backbone.downsample_layers[0](x1) # [1,128,128,128]# 第一个embedding层
            bs1, _, H1, W1 = x1.shape # here dim is not the final dim1
            x1 = x1.view(bs1, -1, H1*W1).transpose(1, 2) # (B,C,H,W) -> (B,C,N)-> (B,N,C) #[1,128,128,128]->[1,128,16384]->[1,16384,128]
        elif isinstance(self.branch1_backbone, DeiT3):
        ...
        if isinstance(self.branch2_backbone, ConvNeXt):
            x2 = self.branch2_backbone.downsample_layers[0](x2)
            bs2, _, H2, W2 = x2.shape # here dim is not the final dim1
        ...
        for i, layer in enumerate(self.interactions):
            indexes1 = self.branch1_interaction_indexes[i] #[0,2] 
            ...
            # 可能叫这个：downsample_layer (convnext) | 

            indexes2 = self.branch2_interaction_indexes[i]
            branch2_blocks = self.branch1_backbone.blocks[indexes2[0]:indexes2[-1] + 1]\
            ...
            x1, x2, _, _, H1, W1, H2, W2 = layer(x1, x2, #[1,16384,128] [1,16384,128]
                        branch1_blocks, branch2_blocks, # 3个bolck 3个block
            ...
            if not self.with_simple_fpn and i in self.out_interaction_indexes:
                x1_ = x1.transpose(1, 2).view(bs1, -1, H1, W1) # [1,16384,128]-->[1,128,128,128]
                x1_ = getattr(self, f"intermediate_merging_{i}_branch1")(x1_) # 单纯的GroupNorm一下 [1, 128, 128, 128]
                ...
                x2_ = getattr(self, f"intermediate_merging_{i}_branch2")(x2_) # x2的维度，变道和x1一样，最高
                x2_ = x2_.type(torch.float32)
                x2_ = F.interpolate(x2_, size=(H2, W2), mode='bilinear', align_corners=False)
        ...
        # 最终的分支合并。
        # Branch merging
        x1 = x1.transpose(1, 2).view(bs1, self.branch1_backbone.embed_dim, H1, W1) # [2, 576, 1024] --> [2, 1024, 24, 24]
        x1 = self.merge_branch1(x1)   # 特征图维度变到branch1的 [2, 1024, 24, 24]->[2, 1024, 24, 24]            
        x1 = x1.type(torch.float32) 
        x1 = F.interpolate(x1, size=(H2, W2), mode='bilinear', align_corners=False)  # 特征图尺寸变到branch3的
        ...
        # x3 = x3.transpose(1, 2).view(bs3, self.branch3.embed_dim, H3, W3) # [2,1600,384] -> [2, 384, 40, 40]
        # x3 = self.merge_branch3(x3)  # 特征图维度变到branch1的 
        
        out = x1 * self.w1 + x2 * self.w2  # 最终的输出
