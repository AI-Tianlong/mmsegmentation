...
class PIIPTwoBranch(nn.Module):
    def __init__(self, n_points=4, 
        ...
        if 'deit' in branch1['pretrained']:
            self.branch1 = vit_models(**branch1)  # VIT
        else:
            self.branch1 = InternViT6B(**branch1) # InternViT
        ...
        dim1 = self.branch1.embed_dim # branch1：最大的维度
        dim2 = self.branch2.embed_dim # branch2：最小的维度
        assert dim1 >= dim2
        ...
        # branch2的维度，变为branch1的维度，变成最大维度
        ...
        out_dim = dim1  # 最终的维度：branch1的维度，具有最大维度
        self.fpn1 = nn.Sequential(                                               # 两次转置卷积，维度不变，尺寸x4
            nn.ConvTranspose2d(out_dim, out_dim, 2, 2),  
            nn.GroupNorm(32, out_dim),
            nn.GELU(),
            nn.ConvTranspose2d(out_dim, out_dim, 2, 2)
        )
        self.fpn2 = nn.Sequential(nn.ConvTranspose2d(out_dim, out_dim, 2, 2))  # 一次转置卷积，维度不变，尺寸x2
        self.fpn3 = nn.Sequential(nn.Identity())                               # 保持不变
        self.fpn4 = nn.Sequential(nn.MaxPool2d(kernel_size=2, stride=2))       # 一次最大池化，维度不变，尺寸 x0.5
    ...
    def forward(self, x):
        ...
        # 创建新的x1,根据缩放因子 插值。
        if scale_factor_1to2 < 1:  # 512 --> 384
            x1 = F.interpolate(x, scale_factor=scale_factor_1to2, mode='bilinear', align_corners=False)
        else:
            x1 = x.clone()

        x2 = x.clone()  # 原始的分辨率，不变   branch1:图像最小，模型最大  branch2:图像最大，模型最小
        
        x1 = x1.type(self.dtype) # return self.branch1.patch_embed.proj.weight.dtype torch.float32
        x2 = x2.type(self.dtype)
        # import pdb; pdb.set_trace()    # 384 / 640 = 1.0 
        ...
        bs1, n1, dim1 = x1.shape  # [2, 576, 3200], 32, 32
                    
        pos_embed1 = self.branch1.pos_embed if not self.branch1_w_cls_token else self.branch1.pos_embed[:, 1:] # [1, 576s, 3200]
        x1 = self.branch1.pos_drop(x1 + pos_embed1) # [1, 576, 3200]

        x2, H2, W2 = self.branch2.patch_embed(x2)   # [2, 1024, 1280],32, 32
        bs2, n2, dim2 = x2.shape                    #  2, 1024, 1280
        pos_embed2 = self.branch2.pos_embed if not self.branch2_w_cls_token else self.branch2.pos_embed[:, 1:] # [1, 1024, 1280]
        x2 = self.branch2.pos_drop(x2 + pos_embed2) # [2, 1024, 1280]

        # Blocks and interactions
        for i, layer in enumerate(self.interactions):  # 12个块儿
            indexes1 = self.branch1_interaction_indexes[i] # config中的interaction_indexes值 [0,3]
            branch1_blocks = self.branch1.blocks[indexes1[0]:indexes1[-1] + 1] # 提取出[0,3]个block 0~3 4个，48/12=4
            indexes2 = self.branch2_interaction_indexes[i] # config中的interaction_indexes值 [0,1]
            branch2_blocks = self.branch2.blocks[indexes2[0]:indexes2[-1] + 1] # 提取出[0,1]个block 0~1 2个，2*4 + 3*8 = 824 = 32
            ...
            # x1:[2,1024,3200] x2:[2,1024,1280]
        
        # Branch merging
        # import pdb; pdb.set_trace()
        x1 = x1.transpose(1, 2).view(bs1, dim1, H1, W1) # [2,576,3200]-->[2,3200,24,24]
        x1 = self.merge_branch1(x1) # [2,3200,24,24]-->[2,3200,24,24]
        x1 = x1.type(torch.float32)
        x1 = F.interpolate(x1, size=(H2, W2), mode='bilinear', align_corners=False) # [2,3200,24,24]-->[2,3200,32,32] s将branch1的小图尺寸恢复最后一个branch的尺寸
        x1 = x1.type(self.dtype)

        # import pdb; pdb.set_trace()
        x2 = x2.transpose(1, 2).view(bs2, dim2, H2, W2) # [2, 1024, 1280] --> [2, 1280, 32, 32]
        x2 = self.merge_branch2(x2) # [2, 1280, 32, 32] --> [2, 3200, 32, 32] # 升维到branch1的维度，最大维度值
        
        out = x1 * self.w1 + x2 * self.w2  # [2, 3200, 32, 32] 最终的输出，等于多个相同纬度和相同大小的特征图利用可学习参数相加
             
        # outputs for fpn
        f1 = self.fpn1(out).contiguous().float()  # x4
        f2 = self.fpn2(out).contiguous().float()  # x2
        f3 = self.fpn3(out).contiguous().float()  # x1
        f4 = self.fpn4(out).contiguous().float()  # x0.5   构建多尺度特征图
        return [f1, f2, f3, f4]
