...
class atl_multi_embedding_VisionTransformer(BaseBackbone):
    """Vision Transformer.
    ...
    def __init__(self,
        ...
        _patch_cfg_RGB.update(patch_cfg)
        _patch_cfg_RGBNIR.update(patch_cfg)
        _patch_cfg_S2MSI.update(patch_cfg)


        self.patch_embed_RGB_3chan = PatchEmbed(**_patch_cfg_RGB)   # 返回 x, out_size   3chan
        self.patch_embed_MSI_4chan = PatchEmbed(**_patch_cfg_RGBNIR)   # 返回 x, out_size  4chan
        self.patch_embed_MSI_10chan = PatchEmbed(**_patch_cfg_S2MSI)   # 返回 x, out_size  10chan

        self.patch_resolution = self.patch_embed_RGB_3chan.init_out_size          # 全部为512，其他的不变
    ...
    def _prepare_pos_embed(self, state_dict, prefix, *args, **kwargs):
        ...
        if self.pos_embed.shape != ckpt_pos_embed_shape:
            ...
            pos_embed_shape = self.patch_embed_RGB_3chan.init_out_size
    ...
    def forward(self, x):
        ...
        # For RGB: [2,3,512,512]   8 bit
        # For RGBNIR: [2,4,512,512] 16 bit
        # For S2MSI: [2,10,512,512] 32 float
        ...
        if C == 3:
            x, patch_resolution = self.patch_embed_RGB_3chan  # (128, 196, 1024), (14, 14)
        elif C == 4:
            x, patch_resolution = self.patch_embed_MSI_4chan
        elif C == 10:
            x, patch_resolution = self.patch_embed_MSI_10chan
        ...
        # import pdb;pdb.set_trace()
        return tuple(outs) #[2,197,1024]  # [batch, num_patches, embed_dims]
