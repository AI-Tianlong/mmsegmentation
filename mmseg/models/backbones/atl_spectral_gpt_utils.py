...
from .atl_spectral_gpt_video_vit import Attention, Block, PatchEmbed
...
class VisionTransformer(BaseModule):
    ...
    def init_weights(self):
        print(f'【ATL-LOG】进入到 SpectralGPT-VisionTransformer init_weights 初始化')
        # import pdb;pdb.set_trace()
        if isinstance(self.init_cfg, dict) and \
            ...
            if self.init_cfg.get('type') == 'Pretrained':
                if 'state_dict' in checkpoint:
                    ...
                    print_log(f'【ATL-log】checkpoint 中的keys: {checkpoint.keys()}')
                    print_log(f'【ATL-log】state_dict 中的keys: {state_dict.keys()}')
                    print_log(f"【ATL-log】已正常从 checkpoint['model'] 导入权重")
                else:
    ...
    def forward(self, x1):
        ...
        x = x1
        # x = x[:, :-1, :, :]  # 切片处理数据维度
        # RuntimeError: Given groups=1, weight of size [768, 12, 3, 8, 8], expected input[2, 1, 12, 512, 512] to have 12 channels, but got 1 channels instead
        x = torch.unsqueeze(x, dim=1) # [2,1,10,512,512]  
        ...
        for i, layer in enumerate(self.blocks):
            ...
            if i in self.out_indices:
                if self.cls_embed:
                    ...
                    out = x
                B, _, C = out.shape  # B, 1024, 768
                # import pdb;pdb.set_trace()
                out = out.view([N, T, L, C]) #[2,4,256,768]
                out = out.permute(0, 2, 3, 1) #[2,256,768,4]
                out = self.fc(out) #[2,256,768,1]
                out = out.reshape(B, self.img_size//self.patch_size, self.img_size//self.patch_size, #128/8
        ...
        # B = x.shape[0] #【2,1024,768】
        # # xx1 = x.view([N, T, L, C])
        # seg1 = seg1.view([N, T, L, C]) #[2,4,256,768]
        # seg1 = seg1.permute(0, 2, 3, 1) #[2,256,768,4]
        # seg1 = self.fc(seg1) #[2,256,768,1]
        # seg1 = seg1.reshape(B, 16, 16, 768).permute(0, 3, 1, 2).contiguous() #[2,768,16,16] 16*8=128


        # m = {}

        # # m[0] = self.conv0(x)  # 256,128,128
        # m[0] = self.conv0(seg1)  # 256,128,128 #[2,256,128,128]

        # # m[1] = self.conv1(x)  # 512,64,64
        # m[1] = self.conv1(seg1)  # 512,64,64   #[2,512,64,64]

        # # m[2] = self.conv2(x)  # 1024,32,32
        # m[2] = self.conv2(seg1)  # 1024,32,32 # [2, 1024, 32, 32]

        # # m[3] = self.conv3(x)  # 2048,16,16
        # m[3] = self.conv3(seg1)  # 2048,16,16 # [2, 2048, 16, 16]

        # m = list(m.values())  #[2,256,128,128] #[2,512,64,64] #[2, 1024, 32, 32] #[2, 2048, 16, 16]
        # x = self.decoder(m)  # #[2,256,128,128]
        # x = self.cls_seg(x)    #[2,13, 128,128]
        # # x = self.sm(x)
        # import pdb;pdb.set_trace()
        # # return {'out': x} # [2,13,128,128] 嗯？ 13是类别。。。
        # import pdb;pdb.set_trace()
        return outs
