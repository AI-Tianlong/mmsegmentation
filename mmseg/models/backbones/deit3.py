...
from mmpretrain.models.utils import (LayerScale, MultiheadAttention, build_norm_layer,
...
class DeiT3TransformerEncoderLayer(BaseModule):
    ...
    def forward(self, x):
        ...
        # x = self.ffn(self.ln1(x), identity=x)  # 原来的，没梯度
...
class DeiT3(VisionTransformer): # copy from mmpretrain.models.backbones.deit3
    ...
    def __init__(self,
                 ...
                 drop_path_rate=0.,
                 qkv_bias=True,
                 norm_cfg=dict(type='LN', eps=1e-6),  # 就是这里 backbone.layers.11.ln2.bias, backbone.layers.11.ln2.weight 没有梯度，看一下为啥？
                 final_norm=True, #
                 out_type='cls_token',
                 with_cls_token=True,
                 use_layer_scale=True,
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            # import pdb;pdb.set_trace()
            if i in self.out_indices:           # x:[B,1025M,384] (32,32)-->[B,384,32,32]
