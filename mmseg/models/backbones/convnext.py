...
class ConvNeXtBlock(BaseModule):
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            shortcut = x               # [2, 128, 160, 160]
            x = self.depthwise_conv(x) # [2, 128, 160, 160] #这就是dwconv

            if self.linear_pw_conv:
                x = x.permute(0, 2, 3, 1)  # (N, C, H, W) -> (N, H, W, C) [2, 128, 160, 160] -> [2, 160, 160, 128]
                x = self.norm(x, data_format='channel_last') # [2, 160, 160, 128]
                x = self.pointwise_conv1(x) # [2, 160, 160, 512]
                x = self.act(x)             # [2, 160, 160, 512]
                if self.grn is not None:
                    x = self.grn(x, data_format='channel_last')    # [2, 160, 160, 512]        
                x = self.pointwise_conv2(x) # [2, 160, 160, 128]        
                x = x.permute(0, 3, 1, 2)  # (N, H, W, C) -> (N, C, H, W) # [2, 160, 160, 128] -> [2, 128, 160, 160]        
            else:
...
class ConvNeXt(BaseBackbone):
    ...
    arch_settings = {
        ...
        'base': {
            'depths': [3, 3, 27, 3],           # 3+3+27+3= 36层+3个下采样层
    ...
    def __init__(self,
        ...
        self.num_stages = len(self.depths) # 4
        ...
        stem = nn.Sequential(  # 第一个下采样层，10->128 channel 减半
            nn.Conv2d(
                in_channels,
                self.channels[0],
        ...
        for i in range(self.num_stages): # 下采样 3 下采样 3 下采样 27 下采样 3 下采样
            depth = self.depths[i]
            channels = self.channels[i] # [128,256,512,1024]
            ...
            stage = Sequential(*[
                ConvNeXtBlock(
                    ...
                    use_grn=use_grn,
                    with_cp=with_cp) for j in range(depth)  # 一个块depths层 3 3 27 3
            ])
            block_idx += depth
        ...
        downsample_ratios=[  
            4, 4, 4,                              # 3
            8, # downsampling layer               # 1
            8, 8, 8,                              # 3
            16, # downsampling layer              # 1
            16, 16, 16, 16, 16, 16, 16, 16, 16,   # 9
            16, 16, 16, 16, 16, 16, 16, 16, 16,   # 9
            16, 16, 16, 16, 16, 16, 16, 16, 16,   # 9
            32, # downsampling layer              # 1
            32, 32, 32]      
        ...
        for i, stage in enumerate(self.stages):
            if i >= 1:
                self.blocks.append(self.downsample_layers[i])
            for convnext_block in stage:
                self.blocks.append(convnext_block)
        ...
        self.embed_dim = self.channels[-1]

    def forward(self, x):
        outs = []
        for i, stage in enumerate(self.stages):  # x: [2, 4, 640, 640]
            x = self.downsample_layers[i](x)     # x: [2, 128, 160, 160] #先过下采样，再提特征
        ...
        # import pdb; pdb.set_trace()          
        return tuple(outs)   # [2, 128, 160, 160] [2, 256, 80, 80] [2, 512, 40, 40] [2, 1024, 20, 20]
