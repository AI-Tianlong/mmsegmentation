...
class EncoderDecoder_TransLU(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 # backbone
                 branch1_backbone_NewTask: ConfigType,
                 branch2_backbone_LCLU: ConfigType,
                # decode_head
                 branch1_decode_head_NewTask: ConfigType,
                 branch2_decode_head_LCLU: ConfigType,
                 
                 # neck
                 branch1_neck_NewTask: OptConfigType = None,
                 branch2_neck_LCLU: OptConfigType = None,
        ...
        self.branch1_backbone_NewTask = MODELS.build(branch1_backbone_NewTask)
        self.branch2_backbone_LCLU = MODELS.build(branch2_backbone_LCLU)
    
        # Build multi decode_head 
        self.branch1_decode_head_NewTask = MODELS.build(branch1_decode_head_NewTask)
        self.branch2_decode_head_LCLU = MODELS.build(branch2_decode_head_LCLU)

        # Build multi neck
        if branch1_neck_NewTask is not None:
            self.branch1_neck_NewTask = MODELS.build(branch1_neck_NewTask)

        if branch2_neck_LCLU is not None:
            self.branch2_neck_LCLU = MODELS.build(branch2_neck_LCLU)

        # 在这里设置不需要梯度（即冻结）, 成功的，在loss里面确实没有梯度，但是这里设置了eval,会被IterBaseTrainLoop给覆盖掉
        self.branch2_backbone_LCLU.eval()
        for param in self.branch2_backbone_LCLU.parameters():
            param.requires_grad = False

        self.branch2_decode_head_LCLU.eval()
        for param in self.branch2_decode_head_LCLU.parameters():
            param.requires_grad = False

        self.branch1_out_channels = self.branch1_decode_head_NewTask.out_channels
        self.branch2_out_channels = self.branch2_decode_head_LCLU.out_channels # 这是18?

        self.align_corners = self.branch2_decode_head_LCLU.align_corners
    ...
    def extract_feat(self, inputs: Tensor) -> List[Tensor]:
        ...
        x_NewTask = self.branch1_backbone_NewTask(inputs)
        x_LCLU = self.branch2_backbone_LCLU(inputs)
    ...
    def encode_decode(self, inputs: Tensor,
        ...
        NewTask_seg_logits = self.branch1_decode_head_NewTask.predict(x_LCLU, batch_img_metas, self.test_cfg)
        LCLU_seg_logits = self.branch2_decode_head_LCLU.predict(x_LCLU, batch_img_metas, self.test_cfg)
        
        return LCLU_seg_logits
    ...
    def _decode_head_forward_train(self, inputs: List[Tensor],
        ...
        losses = dict()
        loss_decode = self.branch1_decode_head_NewTask.loss(inputs, data_samples, self.train_cfg)
    ...
    def loss(self, inputs: Tensor, data_samples: SampleList) -> dict:
        ...
        # self.branch2_decode_head_LCLU.test_output_level='L1'
        # x_LCLU_seglogits_L1 = self.branch2_decode_head_LCLU.predict(x_LCLU, batch_img_metas, self.test_cfg) # 1, 4, 512, 512
        # self.branch2_decode_head_LCLU.test_output_level='L2'
        # x_LCLU_seglogits_L2 = self.branch2_decode_head_LCLU.predict(x_LCLU, batch_img_metas, self.test_cfg) # 1, 9, 512, 512
        # self.branch2_decode_head_LCLU.test_output_level='L3'
        # x_LCLU_seglogits_L3 = self.branch2_decode_head_LCLU.predict(x_LCLU, batch_img_metas, self.test_cfg) # 1, 18, 512, 512

        # x_LCLU_seglogits_list = [x_LCLU_seglogits_L1, x_LCLU_seglogits_L2, x_LCLU_seglogits_L3]
