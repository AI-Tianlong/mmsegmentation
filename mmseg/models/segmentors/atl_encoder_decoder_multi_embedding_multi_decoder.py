...
class ATL_Multi_Embedding_Multi_Decoder_EncoderDecoder(BaseSegmentor):
    ...
    def encode_decode(self, 
        ...
        if inputs.shape[1]==3:
            seg_logits = self.decode_head_MSI_3chan.predict(x, batch_img_metas,
                                              self.test_cfg)
        elif inputs.shape[1]==4:
            seg_logits = self.decode_head_MSI_4chan.predict(x, 
                                              batch_img_metas,
                                              self.test_cfg)
        elif inputs.shape[1]==10:
            seg_logits = self.decode_head_MSI_10chan.predict(x, 
                                              batch_img_metas,
                                              self.test_cfg)
        return seg_logits
