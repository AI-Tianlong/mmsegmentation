...
class EncoderDecoder_TransLU_only_CDKS(BaseSegmentor):
...
# CDKS+CDSA 全功能
    ...
    def __init__(self,
        ...
        self.backbone = MODELS.build(backbone)        # backbone_TransLU
        self.decode_head = MODELS.build(decode_head)  # decode_head (普通的uperhead)
    ...
    def extract_feat(self, inputs: Tensor) -> List[Tensor]:
        ...
        x_NewTask, x_LCLU = self.backbone(inputs) #这个没问题
    ...
    def encode_decode(self, inputs: Tensor,
        ...
        x_NewTask, x_LCLU = self.extract_feat(inputs) #没问题
        
        # 这里，只调用branch1的 decode_head和 branch1的特征
        seg_logits = self.decode_head.predict(x_NewTask, batch_img_metas, self.test_cfg)
    ...
    def _decode_head_forward_train(self, inputs: List[Tensor],
        ...
        loss_decode = self.decode_head.loss(inputs=inputs, 
                                            ...
                                            train_cfg=self.train_cfg) # twobranch_decode_mode2
    ...
    def loss(self, inputs: Tensor, data_samples: SampleList) -> dict:
        ...
        # 验证参数是否冻结
        # print("Checking if branch2_backbone is frozen...")
        for name, param in self.backbone.branch2_backbone.named_parameters():
            if param.requires_grad:
                print(f"Warning: {name} is NOT frozen!")
        # print("Branch2 backbone parameters are successfully frozen.")

        # import pdb; pdb.set_trace()
        # 
        x_NewTask, x_LCLU = self.extract_feat(inputs) #没问题 #[4,10,512,512] --> [[2, 128, 128, 128] [2, 256, 64, 64] [2, 512, 32, 32] [2, 1024, 16, 16]] *2
        
        losses = dict()    # 所以我需要去定义decode_head_forward的loss + forward
        loss_decode = self._decode_head_forward_train(x_NewTask, data_samples)
    ...
    def whole_inference(self, inputs: Tensor,
        ...
        seg_logits = self.encode_decode(inputs, batch_img_metas) #这里得改吧。(不用)
