...
class HSC_LOSS_crop10m(nn.Module):
    ...
    def __init__(self,
        ...
        self.cross_entropy_loss = CrossEntropyLoss(loss_name='loss_HCE_ce')
        ...
        C1, C2, C3 = num_classes[0], num_classes[1], num_classes[2]  # 你自己填：L1/L2/L3 类别数（无background）
        self.path_lookup = torch.full((C1, C2, C3), -1, dtype=torch.long)

        for pid, (l1, l2, l3) in enumerate(self.valid_paths.tolist()):
            self.path_lookup[l1, l2, l3] = pid
    ...
    def forward(self,
        ...
        #     # print(f'loss_ce: {loss_ce}, loss_tk: {loss_tk}, alpha: {alpha}, loss: {loss}')

        elif self.mode == 'L_HSC':
            # ======================= L_HCE ==========================
            w1,w2,w3 = 1.0, 1.0, 1.0
            ce_loss_L1 = self.cross_entropy_loss(pred_seg_logits[0],
                                                LCLU_label[0],
            ...
            ce_loss_L3 = self.cross_entropy_loss(pred_seg_logits[2],
                                                ...
                                                weight=None,
                                                ignore_index=self.ignore_index)
            L_HCE = w1*ce_loss_L1 + w2*ce_loss_L2 + w3*ce_loss_L3  

            # ======================= L_HPC ==========================
            # 1) 准备 mask：任意层是 ignore 就忽略该像素

            # import pdb; pdb.set_trace()
            # y1, y2, y3 = hiera_label_list  # (B,H,W) [2,640,640]
            y1 = LCLU_label[0]
            y2 = LCLU_label[1]
            y3 = crop_label

            valid_mask = (y1 != self.ignore_index) & (y2 != self.ignore_index) & (y3 != self.ignore_index) # (B,H,W) [2,640,640](True or False)

            device = pred_seg_logits[0].device
            valid_paths = self.valid_paths.to(device=device)         # (T,3)
            path_lookup = self.path_lookup.to(device=device)         # (C1,C2,C3)

            # 2) GT tuple -> gt_path_idx
            #    对无效像素先 clamp 到 0 防止索引报错，后面用 valid_mask mask 掉
            y1_safe = y1.clone()
            y2_safe = y2.clone()
            y3_safe = y3.clone()
            y1_safe[~valid_mask] = 0
            y2_safe[~valid_mask] = 0
            y3_safe[~valid_mask] = 0

            gt_path_idx = path_lookup[y1_safe.long(), y2_safe.long(), y3_safe.long()]  # (B,H,W), -1 表示不合法 torch.Size([2, 640, 640])
            valid_mask = valid_mask & (gt_path_idx != -1)

            # 如果一个 batch 里全是 ignore/非法，避免 mean() 变 nan
            if valid_mask.any():
                B, _, H, W = pred_seg_logits[0].shape
                T = valid_paths.shape[0]

                # 3) 计算每层 log-prob (B,H,W,C)
                logp1 = F.log_softmax(pred_seg_logits[0], dim=1).permute(0, 2, 3, 1) # torch.Size([2, 640, 640, 4])
                logp2 = F.log_softmax(pred_seg_logits[1], dim=1).permute(0, 2, 3, 1) # torch.Size([2, 640, 640, 9])
                logp3 = F.log_softmax(pred_seg_logits[2], dim=1).permute(0, 2, 3, 1) # torch.Size([2, 640, 640, 18])

                # 4) 对每条合法路径 t=(c1,c2,c3) 取对应类的 logp 并相加 -> path_scores (B,H,W,T)
                ...
                gt_path_idx_safe[~valid_mask] = 0  # 防止 gather 越界
                nll = -logP.gather(dim=-1, index=gt_path_idx_safe.unsqueeze(-1)).squeeze(-1)  # (B,H,W)

                L_HPC = nll[valid_mask].mean()
            else:
                # 全无效像素时，让 HPC=0（不会影响梯度稳定性）
