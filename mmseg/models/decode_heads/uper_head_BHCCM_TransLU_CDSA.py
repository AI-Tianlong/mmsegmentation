...
class BHCCM_MergeBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.mlp=nn.Sequential(
            nn.Linear(in_features=in_channels, out_features=out_channels, bias=False), # [2,4,1,1]-->[2,9,1,1]
            # nn.ReLU(),
            )
...
class UPerHead_BHCCM_TransLU_CDSA(BaseDecodeHead):
    ...
    def forward(self, inputs):
        ...
        if isinstance(inputs, tuple) and len(inputs)==2 and len(inputs[1])==4:
            
            LCLU_softmask_list = inputs[0]
            barnch1_inputs = inputs[1]

            decode_head_outputs = self._forward_feature(barnch1_inputs)     # [B,128,128,128][B,256,64,64][B,512,32,32][B,1024,16,16] --> [B,512,128,128]
            

            # 在BHCCM_前融合, 把hiera_module 拆开
            # output_list = self.hiera_module(barnch1_inputs, output)  # [B,2,128,128] [B,2,128,128] [B,4,128,128]
            ...
            LCLU_L1_vegetation_seglogits_softmax = LCLU_softmask_list[0].unsqueeze(1) # L1 植被掩膜 # [2, 128, 128]
            LCLU_L2_crop_seglogits_softmax = LCLU_softmask_list[1].unsqueeze(1)       # L2 耕地掩膜 # [2, 128, 128]

            L1_in_other = L1_in[:,0:1,:,:] * (1.0 - LCLU_L1_vegetation_seglogits_softmax)
            L1_in_vegetation = L1_in[:,1:2,:,:] * LCLU_L1_vegetation_seglogits_softmax
            
            L2_in_other = L2_in[:,0:1,:,:] * (1.0 - LCLU_L2_crop_seglogits_softmax)
            L2_in_cropland = L2_in[:,1:2,:,:] * LCLU_L2_crop_seglogits_softmax

            L1_in_final = torch.cat([L1_in_other, L1_in_vegetation], dim=1)
            L2_in_final = torch.cat([L2_in_other, L2_in_cropland], dim=1)
            L3_in_final = L3_in
            # 光有了 输出的特征，还没有监督用的东西，还需要有监督用的东西。
